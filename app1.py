...
def calculate_sop_components(polyol_g):
    ...
    return {
        "polyol_g": polyol_g,
        "c_pentane_g": c_pentane_g,
        "polyol_mixture_g": polyol_mixture_g,
        "mdi_g": mdi_g,
        "total_mixture_g": total_mixture_g,
    }
...
def render_scene(mesh, frac, axis, foam_color, mold_color, mold_opacity, auto_rotate):
    ...
    if axis == "Z":
        origin = (0, 0, zmin + t * (zmax - zmin))
        normal = (0, 0, 1)
    elif axis == "Y":
        origin = (0, ymin + t * (ymax - ymin), 0)
        normal = (0, 1, 0)
    else: # X
        origin = (xmin + t * (xmax - xmin), 0, 0)
        normal = (1, 0, 0)
...
with st.sidebar:
    ...
    V_mold_ml = st.number_input("Mold Volume (mL)", 0.0, value=1000.0, step=10.0)
...
# ------------------------------------------------------------------
# MAIN TABS - ADDING THE NEW PLANNER TAB
# ------------------------------------------------------------------
...
with tab_planner:
    ...
    st.caption("Enter component dimensions to generate a complete production plan using SOP-derived ratios.")
    with st.form("planner_form"):
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Component Dimensions")
            length_mm = st.number_input("Length (mm)", min_value=0.1, value=100.0, step=10.0)
            width_mm = st.number_input("Width (mm)", min_value=0.1, value=100.0, step=10.0)
            thickness_mm = st.number_input("Thickness (mm)", min_value=0.1, value=10.0, step=1.0)
        with col2:
    ...
    if submitted:
        # Calculations based on SOP ratios and target density
        comp_vol_cm3 = (length_mm / 10.0) * (width_mm / 10.0) * (thickness_mm / 10.0)
        ...
        ratio_polyol_to_total_mix = 300.0 / (300.0 + 42.6)
        ratio_cpentane_to_total_mix = 42.6 / (300.0 + 42.6)

        # Back-calculate component masses
        mass_polyol_mix = total_foam_mass_g / (1 + ratio_mdi_to_polyol_mix)
        mass_mdi = total_foam_mass_g - mass_polyol_mix
        mass_polyol = mass_polyol_mix * ratio_polyol_to_total_mix
        mass_cpentane = mass_polyol_mix * ratio_cpentane_to_total_mix

        # Estimate total liquid volume for injection rate
        sg_polyol = 1.05; sg_cpentane = 0.75; sg_mdi = 1.13
        vol_polyol = mass_polyol / sg_polyol
        vol_cpentane = mass_cpentane / sg_cpentane
        vol_mdi = mass_mdi / sg_mdi
        total_liquid_vol_ml = vol_polyol + vol_cpentane + vol_mdi
        ...
        with st.spinner("AI is generating your step-by-step plan..."):
            ...
            if not api_key:
                ...
                try:
                    ...
                    prompt = f"""
                    As a Foam Process Engineer, create a step-by-step production plan for a polyurethane component.
                    The plan must be practical, clear, and based **only** on the data provided below.
                    **Component & Process Data:**
                    - Component Dimensions: {length_mm} mm x {width_mm} mm x {thickness_mm} mm
                    - Total Foam Volume Needed (with {planner_safety_margin}% safety margin): {total_vol_cm3:.1f} cm³ (mL)
                    - Target Density: 23.6 ± 1.0 kg/m³
                    - Material Temperature: 25.0 ± 0.3 °C
                    - Calculated Injection Rate: {injection_rate_ml_s:.2f} mL/second (to meet {target_fill_time_s}s fill time)
                    - Estimated Curing Time (for handling): 20-25 minutes
                    **Required Material Weights (Calculated from SOP Ratios):**
                    - **Polyol:** {mass_polyol:.1f} g
                    - **c-Pentane:** {mass_cpentane:.1f} g
                    - **MDI (Isocyanate):** {mass_mdi:.1f} g
                    **Instructions for the AI:**
                    1.  Start with a summary table of the key parameters.
                    2.  Provide a clear, ordered list for preparing the materials.
                    3.  Give a step-by-step guide for mixing, injection, and curing.
                    4.  Remind the user to perform quality control checks on the final part.
                    5.  Be concise and use markdown for formatting.
                    """
...
with tab_sop_calc:
    ...
    with st.expander("About These Ratios"):
        ...
        """)
    polyol_input_g = st.number_input("Enter Total Polyol Weight (g)", min_value=0.0, value=300.0, step=10.0)
    if polyol_input_g > 0:
...
with tab_sop_qc:
    ...
    with st.expander("How This Calculator Works and What Each Value Means", expanded=True):
        ...
        - **What affects it:** Temperature, component ratios, and moisture.

        #### 3. Foam Density (kg/m³)
        - **What:** The mass per unit of volume, calculated by water displacement.
        - **Why it's critical:** Density controls the foam's strength, insulation value, and cost.
        - **What affects it:** Expansion ratio, temperature/pressure, and blowing agent.

        #### 4. Final Evaluation Steps
        - **What:** The procedural steps for recording and reporting your results.
        - **Why:** This ensures **traceability** and **process control**.
        """)
